...
const makeTokens = (dark) => dark ? {
  ...
  approvalDateBg:'rgba(10,14,26,0.5)', pendingDateBg:'rgba(245,158,11,0.08)',
  declinedDateBg:'rgba(239,68,68,0.06)',
} : {
  ...
  approvalDateBg:'#f8fafc', pendingDateBg:'#fffbeb',
...
const Toast = ({ message }) => {
  ...
  return (
    <div style={{position:'fixed',top:24,right:24,zIndex:9999,padding:'14px 20px',background:err?'linear-gradient(135deg,#ef4444,#dc2626)':'linear-gradient(135deg,#10b981,#059669)',borderRadius:10,color:'#fff',boxShadow:`0 8px 30px ${err?'rgba(239,68,68,.4)':'rgba(16,185,129,.4)'}`,fontSize:13,fontWeight:600,maxWidth:340,animation:'toastIn .3s ease',display:'flex',alignItems:'center',gap:10}}>
...
export default function App() {
  ...
  const writeAuditLog = async (userId,operatorName,actionType,targetContext) => {
    try { await supabase.from('audit_logs').insert([{user_id:userId,operator_name:operatorName,action_type:actionType,target_context:targetContext}]); }
      ...
      <div style={{minHeight:'100vh',position:'relative',zIndex:1}}>
        ...
        <main style={{padding:'28px 24px',maxWidth:1280,margin:'0 auto'}} className="fade-in">
          ...
          {adminTab==='dashboard'&&(() => {
            ...
            const monthFiltered=filtered.filter(act=>{
              const ed=new Date(act.date+'T00:00:00');
              ...
              <div>
                ...
                {monthFiltered.length===0?(
                  ...
                  <div style={{display:'flex',flexDirection:'column',gap:10}}>
                    {monthFiltered.map(act=>(
                      <ActivityCard key={act.id} act={act} onClick={()=>openActivityModal(act.date,act)}/>
                    ))}
                  </div>
                )}
              </div>
          ...
          {adminTab==='myactivities'&&!isAdmin(currentUser)&&!isBishop(currentUser)&&(() => {
            ...
            return (
              <div>
                ...
                <Section title="✕ Declined"        color="#ef4444" items={declined} emptyMsg="No declined activities."/>
              </div>
            );
          })()}
